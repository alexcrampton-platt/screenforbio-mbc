...
sub readSeqidNodeInfoFileWithStartProb {
    my ($file) = @_;
    my @seqids = ();
    my @startnodes = ();
    my @rnodes = ();
    my @nodeinfos = ();
    my ($id,$startnode,$startprob,$n,$i,@a,$rnode,$nodeinfo);
    open(FD,$file) or die "ERROR (readSeqidFileNodeInfoWithStartProb): cannot read file '$file'. $!\n";
    my $linenum=0;
    while (<FD>) {	
	@a=split;
	$linenum++;
	...
	$id = shift(@a);
	push(@seqids,$id);
	push(@nodeinfos,$nodeinfo);
	push(@rnodes,$rnode);
	$n = scalar(@a)/2;
	if (2*$n != scalar(@a)) {
	    die ("ERROR (readSeqidNodeInfoFileWithStartProb): extra/missing columns (nodeinfo,rnode,seqid, 0 or more [startnode,startprob] pairs), line $linenum, file '$file'.\n");
	}
	my @snodes = ();
	for($i=0; $i<$n; $i++) {
	    $startnode=$a[2*$i];
    ...
    close(FD);
    return (\@seqids, \@startnodes, \@rnodes, \@nodeinfos);
}
...
my ($qidref,$qidstart_taxid_probref, $rnoderef, $nodeinforef) = readSeqidNodeInfoFileWithStartProb($seqidfile);
...
my ($k,$numseqs,$qseqid, %seq_dont_use, $nodeclass, $onerefseq,$seqid);
...
for ($k=0; $k<$numseqs; $k++) {
    ...
    print "classify '$qseqid' $node $nodeinforef->[$k]\n" if ($debug_mode);
    if (scalar(@{$qidstart_taxid_probref->[$k]}) == 0) {
	print FD "$nodeinforef->[$k] $rnoderef->[$k] $qidref->[$k]\n";
	next;
    }
    ...
    %seq_dont_use = ();
    ($nodeclass,$onerefseq) = split(/,/,$nodeinforef->[$k]);

    if ($nodeclass eq "nseq2") {
	$seq_dont_use{$qseqid} = 1;
    }
    ...
    elsif (($nodeclass eq "nseq0") or ($nodeclass eq "unk")) {
	$seq_dont_use{$qseqid} = 1;
	foreach $seqid (@{$n2ridref->{$node}}) {
