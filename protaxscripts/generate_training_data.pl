...
if (scalar(@ARGV) < 7) {
    die "usage: generate_training_data.pl taxonomy_file_with_priors seqid2taxonomy_file taxonomyid2rseqid_file num_trainsamples random_seed fulloutput(yes/no) out_file (debug_mode 0/1)\n";
}
...
for (my $i=1; $i<=$num_trainseqs; $i++) {
	...
	$node = findNearestSameLevelNode($node,$n2pref,$n2cref,$n2lref,$n2sidref,$n2ndref,1,$n2prior);
	@seqs = @{$n2sidref->{$node}};
	$ri = int(rand(scalar(@seqs)));
	$trainseq = $seqs[$ri];
